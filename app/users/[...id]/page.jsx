...
export default async function ProductPage({params}){
    ...
    return products.length > 0 && (
                products.map(({id, name, coverAmount, premium, category, image}) => (
                    <Details noButton name={name} coverAmount={coverAmount} premium={premium} category={category} image={image} key={id} />
                ))
            )
